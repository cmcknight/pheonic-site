...
Building a site with Eleventy is pretty slick and relatively simple, but when you are ready to deploy your content you really need to do some cleanup, minfiication, etc., to ensure that you are using as little bandwidth and storage as possible. You _could_ do that manually every time or build scripts to do that, but there is a better answer if you are somewhat versant with Javascript:  [Gulp](https://gulpjs.org).
